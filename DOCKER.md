...
| `HUB_READ_ONLY` | `false` | Set to `true` to disable every write (settings, secrets, groups, labels, task-labels, user updates) and hide the corresponding editing UI in the dashboard. MCP stays mounted, but its two write tools (`create_label`, `set_task_label`) are hidden/rejected too — see `HUB_NO_MCP` to disable MCP entirely instead. A deployment-level switch — it does not change the admin-password login requirement for reads |
