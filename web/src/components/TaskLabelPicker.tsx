...
import { useCreateLabel, useLabels, useSetTaskLabel, type HubLabel, type TaskRef } from "../lib/labels";
 ...
 *  mutually exclusive. Supports find/create-and-apply the same way GroupPicker does for groups. */
export function TaskLabelPicker({ taskRef, applied }: { taskRef: TaskRef; applied: TaskListItemLabel[] }) {
  ...
  const submitCreate = async () => {
    if (!canCreate) return;
    const label = await createLabel.mutateAsync({ name: trimmed });
